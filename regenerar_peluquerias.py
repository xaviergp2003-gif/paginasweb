...
def main() -> None:
    ...
    if SHEET not in wb.sheetnames:
        log.error("No existe la hoja '%s'", SHEET)
        return
    ws = wb[SHEET]
    updated = 0
    for row in range(2, ws.max_row + 1):
        ...
        if not name:
            continue
        dist = Path(str(html_path)).parent if html_path else DIST_DIR / f"pelu-sitges-{row - 1}"
        if not dist.is_absolute():
            dist = ROOT / dist
        log.info("→ %s", name)

        lead = _find_lead(str(name))
        if not lead:
        ...
        if not copy:
            try:
                copy = generate_copy_peluqueria(lead.name, lead.reviews, city=lead.city or "Sitges")
        ...
        try:
            render_peluqueria_landing(lead=lead, copy=copy, output_dir=dist)
        except Exception as e:
            log.error("  HTML: %s", e)
            continue
